...
exports.clean = function(str, is_image, replacement) {
    ...
    for (var i in never_allowed_str) {
        str = str.replace(new RegExp(i, "gi"), (never_allowed_str[i] === null ? replacement : never_allowed_str[i]));
    }

    //Remove regex patterns that are never allowed
    for (var i in never_allowed_regex) {
        str = str.replace(new RegExp(i, 'gi'), replacement);
    }
